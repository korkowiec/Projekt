...
public:
    ...
    int szybkosc=100;//wyrazone w procentach, 100%==1 sekunda całego obrotu
    Animacja_plynna(const sf::Texture &T,float C=100): texture(T),szybkosc(C){setTexture(texture);}
    Animacja_plynna(const sf::Texture &T,RenderWindow &W, float C=100):window(&W), texture(T),szybkosc(C){setTexture(texture);}
    Animacja_plynna(const std::string &S,float C=100);
    Animacja_plynna(const std::string &S,RenderWindow &W,float C=100);
    void Zmien();
    void Zmien_1();
    void Set_pos(sf::Vector2f(pos1));//wokoł punktu
